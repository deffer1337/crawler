...
def _crash_recovery(path_to_directory_with_states: Union[str, Path]):
    ...
    if states:
        ...
        sys.stdout.write(
            f"You have unfinished crawling with domains {states}.\n"
        ...
        if answer:
            ...
            for crawler_state in crawler_states:
                try:
                    _args = (
                        crawler_state.get("url"),
                        crawler_state.get("depth"),
                        crawler_state.get("count_threads"),
                        crawler_state.get("sites"),
                        crawler_state.get("path"),
                        crawler_state.get("is_archive_option"),
                        crawler_state,
                    )
                except KeyError as key_error_msg:
                    logger.error(
                        f"State {crawler_state.get_name_store_file()} not correct. \n"
...
if __name__ == "__main__":
    FileManager(Path(__file__).resolve().parent).make_directory_if_not_exist("Pages")
    FileManager(Path(__file__).resolve().parent).make_directory_if_not_exist("States")
    logger = logging.getLogger("crawl")
    start = time.perf_counter()
    try:
        data_args = ArgParse(sys.argv[1:]).parse()
        logger.info(
            f"Start crawling {data_args.sites} with depth = {data_args.depth}, "
    ...
    crawler = Crawler(ZipArchive, FileManager)
    ...
    for i in range(len(data_args.sites)):
        proc = Process(
            ...
            args=(
                data_args.sites[i],
                data_args.depth,
                data_args.count_threads[i],
                data_args.sites,
                data_args.path,
                data_args.is_archive_option,
            ),
        )
        processes.append(proc)
        proc.start()
